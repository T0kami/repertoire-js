function main(){
  ...
  function listAll(contactListDiv) {
    ...
    objectStore.index('name').openCursor().onsuccess = function(event) {
      ...
      if (cursor) {
        contactListDiv.innerHTML += ("<div><h2>" + cursor.value.name + "</h2><p>" + cursor.value.phone + "</p><p>" + cursor.value.email + "</p></div>");
  ...
  function search(searchString, contactListDiv){
    ...
    objectStore.index('name').openCursor().onsuccess = function(event) {
      ...
      if (cursor) {
        var response = (cursor.value.name + " " + cursor.value.phone + " " + cursor.value.email).toLowerCase() ;
        if(response.includes(searchString.toLowerCase())){
  ...
  document.querySelectorAll("input[type=submit]")[0].addEventListener("click", function(event){
    ...
    for (let item of contactForm){
      ...
      if (item.type == "text"){
        if (item.id == "name"){
          var value = (item.value).capitalize();
          contactInfos[item.id] = value;
        }else contactInfos[item.id] = item.value;
      }
    }
    add(contactInfos);
  });
  ...
  document.getElementById("delete").addEventListener("click", function(event){
    var r = confirm("Voulez-vous vraiment supprimer tous les élèves ?");
    if (r == true) {
      clearData();
      listAll(contactListDiv);
